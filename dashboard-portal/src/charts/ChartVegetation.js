...
export const vegetationCategoryFilter = {
  "Okres wegetacji": (month) => month >= 4 && month <= 9,
  "Poza wegetacją": (month) => [10,11,12,1,2,3].includes(month),
  "Brak danych": (month) => month === null || isNaN(month),
};
...
export function ChartVegetation({ features, stats, onCategoryClick, selectedCategory }) {
  ...
  const handleClick = (entry) => {
    const next = selectedCategory === entry.name ? null : entry.name;
      ...
      <div className="chart-content" style={{ height: '260px' }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart
            ...
            onClick={ stats ? undefined : (e) => e && e.activeLabel && handleClick({ name: e.activeLabel })}
          >
            ...
            <Bar dataKey="value" barSize={18} radius={[0, 2, 2, 0]} cursor={stats ? "default" : "pointer"}>
